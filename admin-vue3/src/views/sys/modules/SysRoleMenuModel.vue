  ...
  >
    ...
    >
      <el-form-item label="角色id" prop="roleId">
        <el-input v-model.trim="form.info.roleId" placeholder="请输入角色id" />
      </el-form-item>
      <el-form-item label="菜单id" prop="menuId">
        <el-input v-model.trim="form.info.menuId" placeholder="请输入菜单id" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const submitForm = () => {
  ...
  formEl.validate((valid, fields) => {
    if (valid) {
      postAction(`/sys/role/menu/${form.info.id != null ? "update" : "save"}`, {
        ...toRaw(form.info),
      }).then((res) => {
        if (res.success) {
          emit("refresh");
