<template>
    ...
    >
        ...
        >
            ...
            <el-input
                    v-model.trim="form.info.perm"
                    placeholder="请输入权限标识"
            /> </el-form-item>
            <el-form-item label="排序：">
            <el-input
                    v-model.trim="form.info.orderNum"
                    placeholder="请输入排序"
            /> </el-form-item>
...
<script setup>
    ...
    const rules = reactive({
        parentId:[ { required: true, message: "请选择父ID,最上级则为0", trigger: "blur" }],
        ...
        menuType:[ { required: true, message: "请选择菜单类型(0:一级菜单; 1:子菜单:2:按钮权限)", trigger: "blur" }],
        perm:[ { required: true, message: "请选择权限标识", trigger: "blur" }],
        orderNum:[ { required: true, message: "请选择排序", trigger: "blur" }],
        enableFlag:[ { required: true, message: "请选择状态  0：正常   1:禁用", trigger: "blur" }],
    });
